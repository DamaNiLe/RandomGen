...
function switchAlgorithm(algorithm) {
    ...
    updatePageTitle(algorithm);
    
    currentAlgorithm = algorithm;
}

// Función para actualizar el título de la página
function updatePageTitle(algorithm) {
    const titleElement = document.getElementById('page-title');
    const subtitleElement = document.getElementById('page-subtitle');
    
    switch(algorithm) {
        case 'intro':
            titleElement.textContent = 'Generador de Números Aleatorios';
            subtitleElement.textContent = 'Métodos Congruenciales Lineal y Multiplicativo';
            break;
        case 'lineal':
            titleElement.textContent = 'Algoritmo Lineal Congruencial';
            subtitleElement.textContent = 'Xᵢ₊₁ = (a × Xᵢ + c) mod m';
            break;
        case 'multiplicativo':
            titleElement.textContent = 'Algoritmo Multiplicativo Congruencial';
            subtitleElement.textContent = 'Xᵢ₊₁ = (a × Xᵢ) mod m';
            break;
    }
}
...
function initializeEventListeners() {
    ...
    // Toggle de tema (funcionalidad futura)
...
function displayDerivedParams(params) {
    ...
    // Mostrar fórmulas y cálculos paso a paso
    displayFormulasLinear(params);
}
...
function displayDerivedParamsMultiplicative(params) {
    ...
    // Mostrar fórmulas y cálculos paso a paso
    displayFormulasMultiplicative(params);
}

// Función para mostrar fórmulas y cálculos paso a paso del algoritmo lineal
function displayFormulasLinear(params) {
    const k = parseInt(document.getElementById('k-lineal').value);
    const c = parseInt(document.getElementById('c-lineal').value);
    const p = parseInt(document.getElementById('p-lineal').value);
    
    // Fórmula para g
    const gFormula = `g = ln(${p}) / ln(2) = ${Math.log(p).toFixed(4)} / ${Math.log(2).toFixed(4)} = ${params.g.toFixed(4)}`;
    document.getElementById('g-formula-lineal').textContent = gFormula;
    
    // Fórmula para a
    const aFormula = `a = 1 + 4 × ${k} = 1 + ${4 * k} = ${params.a}`;
    document.getElementById('a-formula-lineal').textContent = aFormula;
    
    // Fórmula para m
    const mFormula = `m = 2^${params.g.toFixed(4)} = ${params.m}`;
    document.getElementById('m-formula-lineal').textContent = mFormula;
}

// Función para mostrar fórmulas y cálculos paso a paso del algoritmo multiplicativo
function displayFormulasMultiplicative(params) {
    const k = parseInt(document.getElementById('k-multiplicativo').value);
    const p = parseInt(document.getElementById('p-multiplicativo').value);
    const formula = document.querySelector('input[name="k-formula"]:checked').value;
    
    // Fórmula para g
    const gFormula = `g = ln(${p}) / ln(2) + 2 = ${Math.log(p).toFixed(4)} / ${Math.log(2).toFixed(4)} + 2 = ${(Math.log(p) / Math.log(2)).toFixed(4)} + 2 = ${params.g.toFixed(4)}`;
    document.getElementById('g-formula-multiplicativo').textContent = gFormula;
    
    // Fórmula para a
    let aFormula;
    if (formula === '3+8k') {
        aFormula = `a = 3 + 8 × ${k} = 3 + ${8 * k} = ${params.a}`;
        document.getElementById('a-formula-type-multiplicativo').textContent = '3 + 8 × K';
    } else {
        aFormula = `a = 5 + 8 × ${k} = 5 + ${8 * k} = ${params.a}`;
        document.getElementById('a-formula-type-multiplicativo').textContent = '5 + 8 × K';
    }
    document.getElementById('a-formula-multiplicativo').textContent = aFormula;
    
    // Fórmula para m
    const mFormula = `m = 2^${params.g.toFixed(4)} = ${params.m}`;
    document.getElementById('m-formula-multiplicativo').textContent = mFormula;
}
...
function clearResults(algorithm) {
    ...
    if (algorithm === 'lineal') {
        ...
        // Limpiar fórmulas
        document.getElementById('g-formula-lineal').textContent = '-';
        document.getElementById('a-formula-lineal').textContent = '-';
        document.getElementById('m-formula-lineal').textContent = '-';
        ...
        // Limpiar fórmulas
        document.getElementById('g-formula-multiplicativo').textContent = '-';
        document.getElementById('a-formula-multiplicativo').textContent = '-';
        document.getElementById('m-formula-multiplicativo').textContent = '-';
        document.getElementById('a-formula-type-multiplicativo').textContent = '3 + 8 × K';
    }
